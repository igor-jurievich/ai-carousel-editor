...
// Расширен под новый движок: любые реальные глаголы действия, а не только
// легаси-список детерминированных CTA. «Примените/проверьте/начните» — такие же
// валидные action-глаголы, как «сохраните».
const ACTION_VERB_RE =
  /(?:^|[^\p{L}])(напиши|напишите|сохрани|сохраните|оставьте|отправьте|ответьте|пришлите|подпишитесь|выбери|выберите|примени|примените|проверь|проверьте|начни|начните|сделай|сделайте|попробуй|попробуйте|внедри|внедрите|запишись|запишитесь|забери|заберите|протестируй|протестируйте|используй|используйте|скачай|скачайте|поделись|поделитесь|отметь|отметьте|расскажи|расскажите|задай|задайте)(?=$|[^\p{L}])/iu;
const DIRECT_CTA_RE =
...
// Дополнен честными маркерами механизма («причина», «механизм», «работает так»,
// «дело в») — модель формулирует причинность не только через «потому/из-за».
const MECHANISM_CUE_RE =
  /(?:^|[^\p{L}])(потому|поэтому|из-за|когда|если|чтобы|причин\p{L}*|механизм\p{L}*|работает\s+так|дело\s+в|за\s+счёт|благодаря)(?=$|[^\p{L}])/iu;
const STEP_CUE_RE =
