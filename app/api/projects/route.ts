...
function parseProjectBody(body: unknown) {
  ...
  return {
    id: typeof project.id === "string" && /^[0-9a-f-]{36}$/i.test(project.id) ? project.id : undefined,
    title,
    topic,
...
async function createProjectFallback(
  ...
  await replaceLegacySlideRows(supabase, data.id, project.slides);
...
export async function GET() {
  ...
  if (error) {
    return jsonResponse({ error: "Не удалось загрузить проекты." }, 500);
  }

  return jsonResponse({ projects: (data ?? []).map(summaryFromRow) });
}
...
export async function POST(request: Request) {
  ...
  try {
    project = parseProjectBody(await request.json());
  } catch {
  ...
  if (error) {
    if (isMissingProjectJsonColumns(error)) {
      try {
        return jsonResponse({ project: await createProjectFallback(supabase, user.id, project) }, 201);
      } catch (fallbackError) {
    ...
    return jsonResponse({ error: "Не удалось сохранить проект в Supabase." }, 500);
  }

  return jsonResponse({ project: projectFromJsonRow(data) }, 201);
}
