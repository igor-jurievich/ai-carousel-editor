...
const FORBIDDEN_FIRST_SLIDE_STARTS = [
  "в этой карусели",
  "сегодня",
  "я расскажу",
  "поговорим",
...
function resolveModelAttemptsPerCandidate() {
  ...
  if (!Number.isFinite(raw)) {
    return Math.max(2, DEFAULT_MODEL_ATTEMPTS);
  }
...
function validateCarouselResponse(
  ...
  data.slides.forEach((slide: unknown, i: number) => {
    ...
    const ctaText = normalizeText(record.cta_text, 220);
    const before = normalizeText(record.before, 220);
    const after = normalizeText(record.after, 220);
    ...
    if (role === "example") {
      if (!before || !after) {
        errors.push(`Слайд ${i + 1} (example): нет before/after`);
      }
    } else if (role === "cta") {
      if (!body && !ctaText) {
        errors.push(`Слайд ${i + 1} (cta): нет cta_text`);
      }
    } else if (
      (role === "problem" || role === "amplify" || role === "consequence" || role === "solution") &&
      bullets.length < 2 &&
      !body
    ) {
      errors.push(`Слайд ${i + 1} (${role || "unknown"}): нет bullets`);
    } else if (!body && role !== "hook") {
...
function mapModelSlidesToLegacyShape(
  ...
  return expectedFlow.map((role, index) => {
    ...
    const title = normalizeText(record.title, 180);
    const rawSchemaBullets = Array.isArray(record.bullets)
      ? record.bullets
          .map((item) => sanitizeCopyText(normalizeText(item, BULLET_INPUT_MAX), BULLET_OUTPUT_MAX))
          .filter(Boolean)
      : [];
    const body = normalizeBodyText(record.body, 1200) || formatBulletsBody(rawSchemaBullets);
    const bullets = rawSchemaBullets.length ? rawSchemaBullets : extractBodyBullets(body);
    ...
    if (role === "example") {
      ...
      return {
        type: "example",
        before: sanitizeCopyText(normalizeText(record.before, 140), 122) || parsedExample.before,
        after: sanitizeCopyText(normalizeText(record.after, 140), 122) || parsedExample.after
      };
    }

    const ctaText = sanitizeCopyText(normalizeText(record.cta_text, CTA_SUBTITLE_INPUT_MAX), CTA_SUBTITLE_OUTPUT_MAX);
    return {
      type: "cta",
      title,
      subtitle:
...
function startsWithForbiddenOpening(value: string) {
...
function buildModePromptExamples(mode: ContentMode) {
  if (mode === "instruction") {
    return [
      "- GOOD TITLE: Как приучить щенка пошагово",
      "- BAD TITLE: Нужно быть последовательным и внимательным",
      "- GOOD STEP: Сначала зафиксируйте режим: 6 выходов в день по таймеру",
  ...
  if (mode === "expert") {
    return [
      "- GOOD TITLE: Почему щенок писает дома",
...
function hasForbiddenFirstSlideSubtitle(value: string) {
  const normalized = normalizeText(value, 260).toLowerCase();
  if (!normalized) {
    return true;
  }

  return FIRST_SLIDE_SUBTITLE_BANNED_PATTERNS.some((pattern) => pattern.test(normalized));
}

function hasInstructionHookGrammarRisk(value: string) {
  const normalized = sanitizeTitleValue(value, 96).toLowerCase();
  if (!normalized) {
    return true;
  }

  return /^как\s+сделать\s+[\p{L}-]+(?:ть|ти)\b/iu.test(normalized);
}

function buildFirstSlideTitleForMode(topic: string, mode: ContentMode) {
  const focus = buildCompactTopicFocus(sanitizeTopic(topic), 30).replace(/[«»]/gu, "");
  const capitalizedFocus = upperFirst(focus);

  if (mode === "instruction") {
    return buildInstructionFirstSlideTitle(topic, focus);
  }
  ...
  if (mode === "case") {
    if (containsStem(topic.toLowerCase(), ["подписчик", "подписч", "10к", "блог"])) {
      return "10к подписчиков без хаотичных рывков";
    }

    return limitTitleWords(`Кейс: ${focus} до и после`);
  }
  ...
  if (mode === "social") {
    ...
    if (containsStem(topic.toLowerCase(), ["блог", "блогер", "автор", "подписчик", "подписч"])) {
      return "Блогерство выглядит легче снаружи";
    }

    return "За кадром видно больше";
  }

  const directExpertTitle = buildDirectExpertFirstSlideTitle(topic);
  if (directExpertTitle) {
    return directExpertTitle;
  }

  if (containsStem(focus.toLowerCase(), ["личн", "бренд", "дизайн"])) {
    return "Клиент выбирает дизайнера до портфолио";
  }

  return "Где теряется главный сигнал";
}
...
function buildFirstSlideSubtitleForMode(topic: string, mode: ContentMode) {
  ...
  if (mode === "diagnostic") {
    return "Покажем, где появляется сбой, что проверить и какое действие менять первым.";
  }

  if (mode === "case") {
    return "Смотрим исходную точку, действия и результат, чтобы забрать рабочую логику.";
  }
  ...
  return `Один конкретный признак про «${topicFocus}» покажет, где искать настоящую причину.`;
}

function buildInstructionFirstSlideTitle(topic: string, focus: string) {
  const normalizedTopic = normalizeText(topic, 160).toLowerCase();
  if (isApartmentWeekendPrepTopic(topic)) {
    return "Квартира готова за выходные";
  }

  const directTopic = sanitizeTitleValue(normalizeText(topic, 90), 84);
  const actionFocus = normalizeInstructionActionFocus(focus);
  const actionTitle = limitTitleWords(`План: ${actionFocus} без хаоса`);
  if (/^как\b/iu.test(directTopic) && countWords(actionTitle) >= 4) {
    return actionTitle;
  }

  return actionTitle || "План действий без хаоса";
}

function buildDirectExpertFirstSlideTitle(topic: string) {
  const directTopic = sanitizeTitleValue(normalizeText(topic, 90), 84);
  if (!directTopic) {
    return "";
  }
...
function simplifyDirectHookTopic(value: string) {
  return value
    .replace(/\s+даже\b.*$/iu, "")
    .replace(/\s+при\s+правильн[а-яё]*\b.*$/iu, "")
    .trim();
}

function normalizeInstructionActionFocus(value: string) {
  const normalized = normalizeText(value, 80)
    .replace(/^как\s+/iu, "")
    .replace(/^сделать\s+/iu, "")
    .trim();

  const replacements: Array<[RegExp, string]> = [
    [/^подготовка\b/iu, "подготовить"],
    [/^настройка\b/iu, "настроить"],
    [/^проверка\b/iu, "проверить"],
    [/^упаковка\b/iu, "упаковать"],
    [/^продажа\b/iu, "продать"]
  ];
  ...
  return next || "проверить первый шаг";
}

function isInstructionFirstSlideTitle(title: string) {
  const normalized = sanitizeTitleValue(title, 84).toLowerCase();
  if (!normalized) {
    return false;
  }

  if (hasInstructionHookGrammarRisk(normalized)) {
    return false;
  }

  return (
    /^(как|план|шаги)\b/iu.test(normalized) ||
    /\b(пошаг|по\s+шагам|готов[а-яё]*\s+за\s+выходные|без\s+хаоса)\b/iu.test(normalized)
  );
}
...
function enforceFirstSlidePolicy(
  ...
  const echoesRawTopic = isHookEchoingTopic(title, topic);
  const weakTitle =
    !title ||
    countWords(title) < 3 ||
  ...
  const weakSubtitle =
    ...
    hasForbiddenFirstSlideSubtitle(subtitle) ||
    hasDanglingTail(subtitle);
...
function polishHookSubtitleStyle(value: string, mode: ContentMode, topic: string) {
  ...
  if (
    !next ||
    hasHookSubtitleStyleRisk(next) ||
    containsMetaHookLanguage(next) ||
    hasForbiddenFirstSlideSubtitle(next) ||
    hasDanglingTail(next)
  ) {
    return buildFirstSlideSubtitleForMode(topic, mode);
  }
...
function applyHookEditorialPolish(
  ...
  if (
    hasNonSalesGrammarRisk(hook.title) ||
    hasDanglingTail(hook.title) ||
    countWords(hook.title) < 3 ||
    (mode === "instruction" && hasInstructionHookGrammarRisk(hook.title))
  ) {
    hook.title = buildFirstSlideTitleForMode(topic, mode);
  }
  ...
  if (
    !hook.subtitle ||
    containsMetaHookLanguage(hook.subtitle) ||
    hasForbiddenFirstSlideSubtitle(hook.subtitle) ||
    hasDanglingTail(hook.subtitle)
  ) {
    hook.subtitle = buildFirstSlideSubtitleForMode(topic, mode);
  }
...
function applyNonSalesOpeningStyleGuardrails(
  ...
  for (let index = 0; index < openingCount; index += 1) {
    ...
    if (slide.type === "hook") {
      ...
      slide.subtitle =
        !subtitle ||
        hasOpeningStyleRisk(subtitle) ||
        hasForbiddenFirstSlideSubtitle(subtitle) ||
        hasDanglingTail(subtitle) ||
        !hasConcreteAnchorInCopy(subtitle)
...
function validateNonSalesCommonOutput(slides: CarouselOutlineSlide[], topic: string) {
  ...
  if (hook) {
    ...
    if (hasNonSalesGrammarRisk(hook.title) || hasForbiddenFirstSlideSubtitle(hook.subtitle)) {
      reasons.push("first_slide_copy_template");
    }
  ...
  if (cta && (!/[.!?]\s*$/u.test(cta.subtitle) || countPrimaryCtaActions(cta.subtitle) > 1)) {
    reasons.push("non_sales_cta_incomplete");
  }

  if (example && hasWeakExampleCopy(example, "expert")) {
    reasons.push("non_sales_example_weak");
  }

  if (hasNearDuplicateSlideTitles(slides) || hasThinDuplicateOfHook(slides)) {
    reasons.push("non_sales_duplicate_or_thin_slide");
  }
...
function validateExpertOutput(slides: CarouselOutlineSlide[], topic: string) {
  ...
  } else if (hasSolutionRoleMismatch(solution.bullets)) {
    reasons.push("expert_solution_role_mismatch");
  }
...
function repairModeSpecificOutput(
  ...
  if (mode !== "sales") {
    ...
    if (exampleIndex >= 0 && nextSlides[exampleIndex]?.type === "example") {
      ...
      if (hasWeakExampleCopy(example, mode)) {
        nextSlides[exampleIndex] = buildExampleFallback(topic, mode, options);
      }
    }
  }
  ...
  return mode === "sales" ? nextSlides : repairDuplicateAndThinSlides(nextSlides, topic, options);
}
...
function finalizeSlidesForOutput(
  ...
  nextSlides = stripBodyArrowsForNonSales(nextSlides, mode, topic);
  nextSlides = applyNonSalesFinalGuardrails(nextSlides, mode, topic, options);
  nextSlides = sanitizeFinalSlideContent(nextSlides);
...
function sanitizeFinalSlideContent(slides: CarouselOutlineSlide[]) {
  return slides.map((slide) => {
    if (slide.type === "hook" || slide.type === "cta") {
      return {
        ...slide,
        title: sanitizeTitleValue(sanitizeSlideSurfaceText(slide.title, 84), 84),
        subtitle: sanitizeCopyText(
          sanitizeSlideSurfaceText(slide.subtitle, slide.type === "hook" ? HOOK_SUBTITLE_OUTPUT_MAX : CTA_SUBTITLE_OUTPUT_MAX),
          slide.type === "hook" ? HOOK_SUBTITLE_OUTPUT_MAX : CTA_SUBTITLE_OUTPUT_MAX
        )
      };
    }
    ...
    if (slide.type === "problem" || slide.type === "amplify") {
      return {
        ...slide,
        title: sanitizeTitleValue(sanitizeSlideSurfaceText(slide.title, 84), 84),
        bullets: slide.bullets.map((item) => sanitizeCopyText(sanitizeSlideSurfaceText(item, BULLET_OUTPUT_MAX), BULLET_OUTPUT_MAX))
      };
    }

    if (slide.type === "consequence" || slide.type === "solution") {
      return {
        ...slide,
        title: typeof slide.title === "string" ? sanitizeTitleValue(sanitizeSlideSurfaceText(slide.title, 84), 84) : slide.title,
        bullets: slide.bullets.map((item) => sanitizeCopyText(sanitizeSlideSurfaceText(item, BULLET_OUTPUT_MAX), BULLET_OUTPUT_MAX))
      };
    }
    ...
    if (slide.type === "mistake" || slide.type === "shift") {
      return {
        ...slide,
        title: sanitizeTitleValue(sanitizeSlideSurfaceText(slide.title, 92), 92),
        body: sanitizeCopyText(sanitizeSlideSurfaceText(slide.body ?? "", BODY_BLOCK_OUTPUT_MAX), BODY_BLOCK_OUTPUT_MAX)
      };
    }

    return {
      ...slide,
      before: sanitizeCopyText(sanitizeSlideSurfaceText(slide.before, 122), 122),
      after: sanitizeCopyText(sanitizeSlideSurfaceText(slide.after, 122), 122)
    };
  });
}
...
function stripBodyArrowsForNonSales(
  ...
  return slides.map((slide) => {
    if (slide.type === "hook") {
      return {
        ...slide,
        title: sanitizeTitleValue(stripNonSalesMarkers(slide.title, 84), 84),
        subtitle: stripNonSalesMarkers(slide.subtitle, HOOK_SUBTITLE_OUTPUT_MAX)
      };
    }
      ...
      return {
        ...
        title:
          typeof slide.title === "string"
            ? sanitizeTitleValue(stripNonSalesMarkers(slide.title, 84), 84)
            : "",
        bullets: slide.bullets
          .map((item) => stripNonSalesMarkers(item, BULLET_OUTPUT_MAX))
          .filter(Boolean)
          .slice(0, MAX_BULLETS_PER_SLIDE)
      };
    }
    ...
    if (slide.type === "mistake" || slide.type === "shift") {
      return {
        ...slide,
        title: sanitizeTitleValue(stripNonSalesMarkers(slide.title, 92), 92),
        body: stripNonSalesMarkers(slide.body ?? "", BODY_BLOCK_OUTPUT_MAX)
      };
    }
    ...
    if (slide.type === "example") {
      return {
        ...slide,
        before: stripNonSalesMarkers(slide.before, 122),
        after: stripNonSalesMarkers(slide.after, 122)
      };
    }

    if (slide.type === "cta") {
      const ctaVariants = buildCtaVariants(undefined, topic, mode);
      const title = sanitizeTitleValue(stripNonSalesMarkers(slide.title, 84), 84);
      const subtitle = stripNonSalesMarkers(slide.subtitle, CTA_SUBTITLE_OUTPUT_MAX);
      const safeSubtitle =
        !subtitle || containsDirectMessageCta(subtitle) || countPrimaryCtaActions(subtitle) > 1
          ? ctaVariants.soft
          : subtitle;

      return {
        ...slide,
        title: title || buildCtaTitleFallback(undefined, "", mode),
...
function applyNonSalesFinalGuardrails(
  slides: CarouselOutlineSlide[],
  mode: ContentMode,
  topic: string,
  options?: GenerationOptions
  ...
  return nextSlides.map((slide, index) => {
    ...
    if (slide.type === "hook") {
      ...
      const title =
        ...
        (mode === "instruction" && hasInstructionHookGrammarRisk(slide.title))
          ? buildFirstSlideTitleForMode(topic, mode)
    ...
    if (slide.type === "example") {
      if (hasWeakExampleCopy(slide, mode)) {
        return buildExampleFallback(topic, mode, options);
      }
      return {
        type: "example",
        before: ensureExampleLabel("До", slide.before, 122),
        after: ensureExampleLabel("После", slide.after, 122)
      };
    }
    ...
    if (slide.type === "cta") {
      const ctaVariants = buildCtaVariants(options?.goal, topic, mode);
      const normalizedTitle = sanitizeTitleValue(slide.title, 84);
      const title =
        !normalizedTitle || countWords(normalizedTitle) < 2 || isWeakRoleTitle(normalizedTitle)
          ? buildCtaTitleFallback(options?.goal, topic, mode)
          : normalizedTitle;
      return {
        type: "cta",
        title,
        subtitle: normalizeNonSalesCtaSubtitle(slide.subtitle, ctaVariants.soft)
      };
    }
...
function isApartmentWeekendPrepTopic(topic: string) {
  const normalizedTopic = normalizeText(topic, 180).toLowerCase();
  return (
    isRealEstatePrepTopic(topic) &&
    containsStem(normalizedTopic, ["выходн", "дня", "суббот", "воскрес"])
  );
}

function isRealEstatePrepTopic(topic: string) {
  const normalizedTopic = normalizeText(topic, 180).toLowerCase();
  const hasRealEstateObject = containsStem(normalizedTopic, [
    "квартир",
    "риелтор",
    "недвижим",
    "объект",
  ...
  const hasPrepOrSaleContext = containsStem(normalizedTopic, [
    "подготов",
    "продаж",
    "выходн",
    "фото",
  ...
  return hasRealEstateObject && hasPrepOrSaleContext;
}
...
function normalizeHookSubtitleForOutput(value: string, topic: string, mode: ContentMode) {
  const fallback = buildFirstSlideSubtitleForMode(topic, mode);
  const sentence = normalizeCompleteSentence(value, HOOK_SUBTITLE_OUTPUT_MAX);
  if (
    !sentence ||
    countWords(sentence) < 5 ||
    containsMetaHookLanguage(sentence) ||
    hasForbiddenFirstSlideSubtitle(sentence) ||
    hasDanglingTail(sentence)
  ) {
    return fallback;
  }

  const sentences = sentence.split(/(?<=[.!?])\s+/u).filter(Boolean).slice(0, 2).join(" ");
  return sentences || fallback;
}

function ensureSentenceTerminal(value: string, maxLength: number) {
  const sentence = normalizeCompleteSentence(value, maxLength);
  return sentence || sanitizeCopyText(normalizeText(value, maxLength), maxLength);
}

function ensureExampleLabel(label: "До" | "После", value: string, maxLength: number) {
  const sentence = ensureSentenceTerminal(value, maxLength);
  if (!sentence) {
    return "";
  }

  if (new RegExp(`^${label}:`, "iu").test(sentence)) {
    return sentence;
  }

  return sanitizeCopyText(normalizeText(`${label}: ${sentence}`, maxLength + 20), maxLength);
}

function stripNonSalesMarkers(value: string, maxLength: number) {
  return sanitizeCopyText(normalizeText(value, maxLength), maxLength)
    .replace(/\s*→\s*/gu, " ")
    .replace(/^[•·\-–—→\s]+/u, "")
    .replace(/\s{2,}/gu, " ")
    .trim();
...
export async function generateCarouselFromTopic(
  ...
  try {
    ...
    for (const model of models) {
      let qualityRetryInstruction = "";
      for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
        try {
          const baseUserMessage = buildUserPrompt(
            cleanedTopic,
            options?.niche,
            options?.audience,
            options?.tone,
          ...
          const userMessage = qualityRetryInstruction
            ? `${baseUserMessage}\n\nПОВТОРНАЯ ПОПЫТКА QUALITY GATE:\n${qualityRetryInstruction}`
            : baseUserMessage;
          ...
          const quality = evaluateSlideQuality(
          ...
          if (!quality.ok) {
            ...
            if (attempt < modelAttempts) {
              qualityRetryInstruction = buildQualityRetryInstruction(quality.reasons, modeDecision.modeEffective);
              console.warn(
...
function resolveExpectedFlowByMode(mode: ContentMode, targetCount: number) {
  return buildSlidePlan(mode, targetCount);
}
...
function buildUserPrompt(
  ...
  if (resolvedAudience) {
    prompt += `\nЦелевая аудитория: ${resolvedAudience}. Пиши на языке этой аудитории, без лишней терминологии.`;
  }
  if (resolvedTone) {
    prompt += `\nТон: ${resolvedTone}. Сохрани этот тон во всех заголовках, body и caption.`;
  }
  if (resolvedGoal) {
    prompt += `\nЦель карусели: ${resolvedGoal}. CTA должен соответствовать цели и режиму контента.`;
  }
  ...
  if (mode !== "sales") {
    prompt +=
      ...
      "\nЗапрещено в subtitle первого слайда: «разберём», «в теме», «рассмотрим», «поговорим о»." +
      "\nТон: спокойный экспертный, без обвинений и лишней драматизации." +
      "\nCTA: мягкий, одно полное предложение до 20 слов, без «напиши слово в директ» и без двойного призыва." +
      "\nExample: обязательный конкретный До/После с реалистичными цифрами, без шаблона «объясняли общо»." +
      "\nРоли: solution содержит только решения и действия, problem содержит только проблему.";
  }

  if (mode === "instruction") {
    prompt += "\nФокус структуры: цель, шаги, условия, частые ошибки и краткое резюме.";
  } else if (mode === "diagnostic") {
    prompt += "\nФокус структуры: симптомы, причины, механизм и как исправить.";
  } else if (mode === "case") {
    prompt += "\nФокус структуры: контекст, действия, результат и выводы.";
  }
  ...
  return {
    ...
    properties: {
        ...
        items: {
          ...
          properties: {
            role: {
              ...
              description: "Роль слайда строго по плану. Не меняй порядок ролей."
            },
            title: {
              type: "string",
              maxLength: 80,
              description:
                "Заголовок до 60 символов и максимум 6 слов. Для example верни пустую строку. Не используй Instagram, карусель, пост, слайд."
            },
            body: {
              type: "string",
              maxLength: 900,
              description:
                "Тело слайда 80-160 символов или пункты через перенос строки. Для example верни пустую строку. Конкретика, глаголы, без служебных слов."
            },
            bullets: {
              type: "array",
              minItems: 0,
              maxItems: 4,
              description:
                "Для problem/amplify/consequence/solution верни 2-4 пункта по 30-80 символов. Каждый пункт начинается с глагола или наблюдаемого действия.",
              items: {
                type: "string",
                maxLength: 100
              }
            ...
            before: {
              type: "string",
              maxLength: 140,
              description: "Только для example: ситуация ДО с цифрой или фактом. Для остальных ролей пустая строка."
            },
            after: {
              type: "string",
              maxLength: 140,
              description: "Только для example: ситуация ПОСЛЕ с цифрой или фактом. Для остальных ролей пустая строка."
            },
            cta_text: {
              type: "string",
              maxLength: 100,
              description:
                "Только для cta: одно конкретное действие 40-80 символов. Для остальных ролей пустая строка."
            }
          },
      ...
      caption: {
        type: "string",
        maxLength: 2400,
        description: "Подпись к публикации без хештегов внутри текста."
      }
    },
...
function applyFinalCopyPolish(
  ...
  if (ctaIndex >= 0) {
    ...
    if (cta?.type === "cta") {
      ...
      const shouldUseSoftCta =
        mode !== "sales" &&
        (directCtaDetected || !hasPrimaryAction || countPrimaryCtaActions(normalizedSubtitle) > 1);
...
function enforceSlideQuality(
  ...
  return expectedFlow.map((role, index) => {
    ...
    if (role === "solution") {
      ...
      if (mode !== "sales" && hasSolutionRoleMismatch(strongBullets)) {
        return solutionFallback;
      }
      return {
        type: "solution",
    ...
    if (role === "example") {
      ...
      const exampleFallback = buildExampleFallback(topic, mode, options);
      ...
      const candidate = { type: "example" as const, before, after };
      const beforeLooksWeak = !before || countWords(before) < 4;
      const afterLooksWeak = !after || countWords(after) < 4;
      const exampleLooksWeak = hasWeakExampleCopy(candidate, mode);
      return {
        type: "example",
        before: beforeLooksWeak || exampleLooksWeak ? exampleFallback.before : before,
        after: afterLooksWeak || exampleLooksWeak ? exampleFallback.after : after
      };
    }
    ...
    const shouldUseSoftCta =
      mode !== "sales" &&
      (directCtaDetected || !hasPrimaryAction || countPrimaryCtaActions(subtitle) > 1);
    const actionIsAcceptable = hasPrimaryAction;
    return {
      type: "cta",
      title:
...
function evaluateSlideQuality(
  ...
  for (let index = 0; index < expectedFlow.length; index += 1) {
    ...
    if (role === "solution") {
      ...
      if (mode !== "sales" && hasSolutionRoleMismatch(solution.bullets)) {
        reasons.push("solution_role_mismatch");
      }
      continue;
    }
    ...
    const ctaCopy = `${cta.title} ${cta.subtitle}`.trim();
    const thinTitleWithoutAction = countWords(cta.title) < 2 && !hasActionVerb(cta.title);
    if (thinTitleWithoutAction || !hasActionVerb(ctaCopy)) {
...
function buildQualityRetryInstruction(reasons: string[], mode: ContentMode) {
  const reasonText = dedupeErrors(reasons).join("; ") || "слабая структура";
  const ctaInstruction =
    mode === "sales"
      ? "CTA: одно действие с ключевым словом для сообщения и конкретной пользой в ответ."
      : mode === "social"
        ? "CTA: один живой вопрос или просьба отметить человека, без директ-продажи."
        : "CTA: один мягкий шаг: сохранить, проверить у себя или применить один пункт.";

  return [
    `Исправь причины провала: ${reasonText}.`,
    "Hook: не оглавление; дай боль, факт, цифру, результат или open loop в 4-6 словах. Не повторяй тему как «Как/Почему + тема».",
    "Слайд 2: автономный симптом, который понятен без обложки.",
    "Bullets: начинай с глагола или наблюдаемого действия, без абстракций.",
    "Убери слова Instagram, инстаграм, карусель, пост, слайд, свайп, пользователь из текста слайдов.",
    ctaInstruction,
    "Верни тот же JSON-контракт, без пояснений."
  ].join("\n");
}
...
function hasSolutionRoleMismatch(bullets: string[]) {
  const cleaned = bullets
    .map((item) => normalizeText(item, BULLET_OUTPUT_MAX).toLowerCase())
    .filter(Boolean);
  if (cleaned.length < 2) {
    return false;
  }

  const negativeCount = cleaned.filter((item) =>
    SOLUTION_ROLE_NEGATIVE_PATTERNS.some((pattern) => pattern.test(item))
  ).length;
  const actionCount = cleaned.filter((item) =>
    SOLUTION_ROLE_ACTION_PATTERNS.some((pattern) => pattern.test(item))
  ).length;
...
function hasNearDuplicateSlideTitles(slides: CarouselOutlineSlide[]) {
  const titles = slides
    .map((slide) => ("title" in slide && typeof slide.title === "string" ? slide.title : ""))
    .filter(Boolean);

  for (let i = 0; i < titles.length; i += 1) {
    for (let j = i + 1; j < titles.length; j += 1) {
      if (titleSimilarity(titles[i] ?? "", titles[j] ?? "") >= 0.8) {
        return true;
      }
    }
  }
...
function hasThinDuplicateOfHook(slides: CarouselOutlineSlide[]) {
  const hook = slides.find((slide) => slide.type === "hook");
  if (!hook) {
    return false;
  }
  ...
  return slides.some((slide) => {
    if (slide.type === "hook" || !("title" in slide) || typeof slide.title !== "string") {
      return false;
    }

    return titleSimilarity(hookTitle, slide.title) >= 0.8 && countWords(collectSlideCopy(slide)) < 15;
  });
}

function repairDuplicateAndThinSlides(
  slides: CarouselOutlineSlide[],
  topic: string,
  options?: GenerationOptions
) {
  ...
  const hook = nextSlides.find((slide) => slide.type === "hook");
  const hookTitle = hook?.type === "hook" ? hook.title : "";

  for (let index = 0; index < nextSlides.length; index += 1) {
    const slide = nextSlides[index];
    if (!slide || slide.type === "hook" || !("title" in slide) || typeof slide.title !== "string") {
      continue;
    }

    const slideTitle = slide.title;
    const duplicatesEarlier = seenTitles.some((title) => titleSimilarity(title, slideTitle) >= 0.8);
    const duplicatesHook =
      hookTitle && titleSimilarity(hookTitle, slideTitle) >= 0.8 && countWords(collectSlideCopy(slide)) < 24;

    if (duplicatesEarlier || duplicatesHook) {
      if (slide.type === "cta") {
        slide.title = buildCtaTitleFallback(options?.goal, topic, resolveModeFromOptions(options));
      } else {
        slide.title = buildRoleTitleFallback(slide.type, topic, options);
      }
    }
...
function titleSimilarity(a: string, b: string) {
  const first = new Set(normalizeWordTokens(a).filter((token) => !TOPIC_STOP_WORDS.has(token)));
  const second = new Set(normalizeWordTokens(b).filter((token) => !TOPIC_STOP_WORDS.has(token)));
...
function hasPrimaryCtaActionVerb(value: string) {
  return /(?:^|[^\p{L}])(напиши|напишите|сохраните|оставьте|отправьте|ответьте|пришлите|подпишитесь|выберите|проверьте|сделайте|примените|попробуйте|начните)(?=$|[^\p{L}])/iu.test(
    value
  );
}

function countPrimaryCtaActions(value: string) {
  const matches = normalizeText(value, 260).match(
    /(?:^|[^\p{L}])(напиши|напишите|сохраните|оставьте|отправьте|ответьте|пришлите|подпишитесь|выберите|проверьте|сделайте|примените|попробуйте|начните)(?=$|[^\p{L}])/giu
  );
...
function rescueWeakSlidesAfterQualityCheck(
  ...
  const polished = applyFinalCopyPolish(
  ...
  return applyNonSalesFinalGuardrails(polished, mode, topic, options);
}
...
function shouldPreferDomainFallbackBullets(
  ...
  if (isRealEstatePrepTopic(topic) && role === "solution") {
  ...
  if (mode !== "sales") {
    if (mode === "instruction") {
      return [
        ...
        {
          title: trimToWordBoundary(`План по шагам: ${topicFocus}`, 72),
          subtitle: trimToWordBoundary("Соберём порядок действий, условия и проверку результата.", 132)
        },
        {
          title: trimToWordBoundary(`Как ${topicFocus} без лишнего хаоса`, 72),
          subtitle: trimToWordBoundary("Покажу первый шаг, частые ошибки и критерий готовности.", 132)
        }
      ];
    }
    ...
    const nonSalesSubtitle =
      mode === "diagnostic"
          ? "Покажем симптомы, причины и план исправления без запугивания."
          : mode === "case"
            ? "Покажу контекст, действия и результат на реальном сценарии."
            : mode === "social"
              ? "Коротко разложим, что мешает и что реально помогает в быту."
              : "Покажем причины, механизм и рабочие шаги без агрессивных хуков.";
    ...
    return [
      ...
      {
        title: trimToWordBoundary(`Что мешает стабильно решить ${topicFocus}`, 72),
...
function normalizeSlideByType(
  ...
  if (expectedType === "example") {
    const exampleFallback = buildExampleFallback(sanitizedTopic, mode, options);
    return {
      type: "example",
      before:
        sanitizeCopyText(normalizeText(safe.before, 128), 122) ||
        exampleFallback.before,
      after:
        sanitizeCopyText(normalizeText(safe.after, 128), 122) ||
        exampleFallback.after
    };
  }
...
function buildExampleFallback(
  topic: string,
  mode: ContentMode,
  options?: GenerationOptions
): Extract<CarouselOutlineSlide, { type: "example" }> {
  ...
  if (containsStem(normalizedTopic, ["щен", "собак", "туалет", "луж", "выгул"])) {
    return {
      type: "example",
      before: "До: 4 лужи дома в день даже после долгой прогулки.",
      after: "После: 6 выходов по режиму сократили лужи за 10 дней."
  ...
  if (containsStem(normalizedTopic, ["квартир", "риелтор", "недвижим", "объект", "показ", "циан", "авито"])) {
    ...
    return {
      type: "example",
      before: "До: 6 месяцев показов без задатка и 20 случайных просмотров.",
      after: "После: 9 целевых показов и задаток на 14-й день."
    };
  }
  ...
  if (domain === "sales") {
    return {
      type: "example",
      before: "До: бюджет вырос на 30%, а заявок осталось 12 в неделю.",
      after: "После: отключили 2 связки и снизили CPL на 25% за 10 дней."
    };
  }
  ...
  if (mode === "instruction") {
    return {
      type: "example",
      before: "До: делали без системы, и результат менялся каждые 2 дня.",
      after: "После: пошли по чек-листу и получили стабильность за 7 дней."
    };
  }
  ...
  if (mode === "case") {
    return {
      type: "example",
      before: "До: 3 месяца действий без результата и понятной гипотезы.",
      after: "После: 2 точных изменения дали первый результат за 14 дней."
    };
  }

  return {
    type: "example",
    before: "До: 3 недели пробовали разрозненные действия без понятного эффекта.",
    after: "После: 2 шага по системе дали измеримый результат за 10 дней."
  };
}

function hasWeakExampleCopy(
  example: Extract<CarouselOutlineSlide, { type: "example" }>,
  mode: ContentMode
) {
  const copy = `${example.before} ${example.after}`.trim();
  if (!copy) {
    return true;
  }
...
function sanitizeTopic(topic: string): string {
  ...
  const compact = removeDanglingTail(trimToWordBoundary(cleaned, 48));
  if (compact.length >= 3) {
    return compact;
  }
...
function buildSolutionTitleFallback(topic: string) {
  ...
  if (isRealEstatePrepTopic(topic)) {
...
function buildDeterministicSolutionFallback(
  ...
  const domain = resolveTopicDomain(topic, options);
  const normalizedTopic = normalizeText(topic, 180).toLowerCase();
  const title = buildSolutionTitleFallback(topic);
  ...
  if (domain === "fitness") {
    return {
      ...
      bullets: [
        "Стабилизируйте сон до 7 часов хотя бы 5 дней подряд",
        "Добавьте 7000–9000 шагов без увеличения жестких тренировок",
        "Запланируйте 2 легких дня восстановления каждую неделю"
      ]
    };
  }
  ...
  return {
    ...
    title,
    bullets: [
      "Выберите одно конкретное действие и выполните его сегодня",
      "Проверьте результат через 3 дня по одному понятному признаку",
      "Повторите рабочий шаг 7 дней, прежде чем менять систему"
    ]
  };
}
...
function buildRoleTitleFallback(
  ...
  if (mode === "instruction" && isRealEstatePrepTopic(topic)) {
...
function buildDomainRoleTitleVariants(domain: TopicDomain, focus: string) {
  ...
  if (domain === "creator") {
    return {
      problem: [
        `${focus}: где теряется живой голос`,
        `${focus}: почему внимание падает рано`
      ],
      amplify: [
        `${focus}: как слабый ритм режет интерес`,
        `${focus}: почему польза не становится диалогом`
      ],
      mistake: [
      ...
      shift: [
        `${focus}: как вернуть живой темп`,
        `${focus}: разворот к ясной позиции`
      ]
    };
  }
  ...
  if (domain === "creator") {
    if (role === "problem") {
      return [
        ...
        "Мысль растягивается, и читатель теряет нить уже к середине.",
        "Нет конкретного действия, ради которого человек дочитывает."
      ];
    }
    if (role === "amplify") {
      return [
        "Низкий досмотр сигнализирует алгоритму, что материал слабый.",
        "Полезность есть, но она не превращается в сохранения и диалоги.",
        "Каждая следующая публикация сложнее раскачать на старте."
      ];
    }
    if (role === "consequence") {
      return [
        ...
        "План публикаций есть, а воронка доверия не собирается."
      ];
    }
    return [
      "Делайте один блок = одна мысль с четким микровыводом.",
...
function hasDanglingTail(value: string) {
  return /\b(и|а|но|или|что|чтобы|потому|когда|где|как|если|по|в|на|для|с|к|от|из|из-за|у|до|за|без|при|о|об|обо|над|под|между|через|про|не|хотя|даже|который|которого|которой|которые|первого|второго|третьего|четвертого|пятого|обычно|живой|стабильному)\s*$/iu.test(
    value.trim()
  );
}
...
function hasBrokenCopyTail(value: string) {
  ...
  if (/\b(первых|следующих|ближайших|пробные|тестовые|конкретные|основные|целевые|каждый|каждую|каждое|них|структуре)\s*$/iu.test(normalized)) {
    return true;
  }
...
function removeBrokenCopyTail(value: string) {
  return value
    ...
    .replace(/\b(первых|следующих|ближайших|пробные|тестовые|конкретные|основные|целевые|каждый|каждую|каждое|них|структуре)\s*$/iu, "")
...
function removeDanglingTail(value: string) {
  return value
    .replace(
      /(?:^|[\s\u00A0])(и|а|но|или|что|чтобы|потому|когда|где|как|если|по|в|на|для|с|к|от|из|из-за|у|до|за|без|при|о|об|обо|над|под|между|через|про|не|хотя|даже|который|которого|которой|которые|первого|второго|третьего|четвертого|пятого|обычно|живой|стабильному)\s*$/iu,
      ""
    )
    .replace(/[,:;—–-]+\s*$/u, "")
    .trim();
...
function hasAwkwardHookTitle(value: string) {
  ...
  return (
    ...
    /набрал\s+\d+(?:\s|\u00a0)*(?:к|k|000)\s+людей/iu.test(normalized) ||
...
function hasNonSalesGrammarRisk(value: string) {
  ...
  return (
    /^почему\s+вокруг\b/iu.test(normalized) ||
    /^как\s+сделать\s+[\p{L}-]+(?:ть|ти)\b/iu.test(normalized) ||
...
function sanitizeTitleValue(value: unknown, maxLength: number) {
  ...
  return sanitizeCopyText(limitTitleWords(compactWords.join(" ")), maxLength)
    .replace(/[.!]+\s*$/u, "")
    .trim();
}
...
function hasTopicStemMatch(copy: string, topicStems: Set<string>) {
  ...
  const minTokenLength = Array.from(topicStems).some((stem) => stem.length <= 3) ? 3 : 4;
  const copyTokens = normalizeWordTokens(copy).filter(
    (token) => token.length >= minTokenLength && !TOPIC_STOP_WORDS.has(token)
  );
...
function addTopicAnchorToTitle(title: string, topicAnchor: string) {
  ...
  const anchoredTitle = limitTitleWords(trimToWordBoundary(`${normalizedTitle} — про ${topicAnchor}`, 90));
...
function limitTitleWords(value: string, maxWords = 6) {
  const normalized = value
    .replace(/[«»]/gu, "")
    .replace(/\s+/gu, " ")
    .trim();
  ...
  return removeDanglingTail(words.slice(0, maxWords).join(" ")) || words.slice(0, maxWords).join(" ");
}
...
function normalizeCaption(
  ...
  const safeCtaAggressive =
    mode !== "sales" || containsDirectMessageCta(ctaAggressive) ? safeCtaSoft : ctaAggressive;
  const cta =
    safeRequestedCta || selectCtaByGoal(goal, { soft: safeCtaSoft, aggressive: safeCtaAggressive }, mode);
  ...
  return {
    ...
    cta,
    ctaSoft: safeCtaSoft,
    ctaAggressive: safeCtaAggressive,
...
function buildCtaVariants(goal?: string, topic?: string, mode: ContentMode = "expert") {
  ...
  const soft = isFollowersGoal(normalizedGoal)
    ? pickVariantByTopic(seed, [
        "Сохраните схему, чтобы вернуться к ней перед следующей публикацией.",
        "Сохраните план для спокойной сверки подхода."
      ])
    : buildDomainSoftCta(domain, focus, sanitizedTopic);

  const aggressive =
    mode === "sales"
      ? isLeadsGoal(normalizedGoal)
  ...
  const normalizedAggressive =
    mode === "sales"
      ? trimToWordBoundary(aggressive, 210)
      : normalizeNonSalesCtaSubtitle(aggressive, "Выберите один шаг и примените его сегодня.");
  const normalizedSoft =
    mode === "sales"
      ? trimToWordBoundary(soft, 210)
      : normalizeNonSalesCtaSubtitle(soft, "Выберите один шаг и примените его сегодня.");
  const selected =
    mode === "sales"
...
function normalizeNonSalesCtaSubtitle(value: string, fallback: string) {
  const candidate = normalizeCompleteSentence(value, CTA_SUBTITLE_OUTPUT_MAX, 20);
  const fallbackSentence =
    normalizeCompleteSentence(fallback, CTA_SUBTITLE_OUTPUT_MAX, 20) ||
    "Выберите один шаг и примените его сегодня.";

  if (
    !candidate ||
    countWords(candidate) < 4 ||
    containsDirectMessageCta(candidate) ||
    countPrimaryCtaActions(candidate) > 1 ||
    hasRawTopicCtaLanguage(candidate)
  ) {
...
function normalizeCompleteSentence(value: string, maxLength: number, maxWords?: number) {
  ...
  const body = removeDanglingTail(firstSentence.replace(/[.!?]+\s*$/u, "").trim());
  if (!body || countWords(body) < 2) {
    return "";
  }

  const limitedBody =
    typeof maxWords === "number" && countWords(body) > maxWords
      ? removeDanglingTail(body.split(/\s+/u).slice(0, maxWords).join(" "))
      : body;
  if (!limitedBody || countWords(limitedBody) < 2) {
    return "";
  }

  const terminal = /[!?]\s*$/u.test(firstSentence) ? firstSentence.trim().slice(-1) : ".";
  return `${limitedBody}${terminal}`;
}

function hasRawTopicCtaLanguage(value: string) {
  const normalized = normalizeText(value, 220).toLowerCase();
  if (!normalized) {
    return false;
  }

  return /(?:^|[^\p{L}])сохраните\s+разбор\s+по(?=$|[^\p{L}])/iu.test(normalized);
}
...
function buildDomainSoftCta(domain: TopicDomain, focus: string, topicSeed?: string) {
  ...
  const variantsByDomain: Record<TopicDomain, string[]> = {
    ...
    education: [
      "Сохраните план для подготовки ближайшего урока.",
      "Выберите один шаг для следующего занятия."
    ],
    psychology: [
      "Сохраните схему для следующей консультации.",
      "Выберите один вопрос для ближайшей сессии."
    ],
    health: [
      "Сохраните памятку для спокойной проверки шагов.",
      "Выберите один шаг, который можно внедрить сегодня."
    ],
    fitness: [
      "Сохраните план для первого шага на этой неделе.",
      "Выберите один шаг для ближайшей тренировки."
    ],
    beauty: [
      "Сохраните разбор для работы с ближайшим клиентом.",
      "Выберите один шаг для следующей записи."
    ],
    finance: [
      "Сохраните чек-лист для следующего финансового решения.",
      "Выберите один шаг, который снизит тревогу в финансах."
    ],
    creator: [
      "Выберите пункт, который внедрите в ближайшей публикации.",
      "Сохраните план для следующей публикации."
    ],
    sales: [
      "Сохраните план для первого рабочего шага сегодня.",
      "Выберите один пункт для воронки на этой неделе."
    ],
    general: [
      "Выберите первый шаг, который внедрите сегодня.",
      "Сохраните карусель для спокойной проверки."
    ]
  };
