...
Структура:
- слайд 1: "Как сделать X пошагово" или прямой эквивалент;
...
function buildFirstSlideTitleForMode(topic: string, mode: ContentMode) {
  ...
  if (mode === "instruction") {
    const actionFocus = /(?:ть|ти)\b/iu.test(focus) ? focus : `сделать ${focus}`;
    return limitTitleWords(`Как ${actionFocus} пошагово`);
  }
...
function buildFirstSlideSubtitleForMode(topic: string, mode: ContentMode) {
  ...
  if (mode === "instruction") {
    return trimToWordBoundary(
      `Дам пошаговый план по теме ${focus}: что сделать сегодня и что проверить дальше.`,
      148
    );
  }
  ...
  if (mode === "diagnostic") {
    return trimToWordBoundary(
      `Разберём причины срывов в теме ${focus} и покажем, как их исправить без перегруза.`,
      148
    );
  }
  ...
  if (mode === "case") {
    return trimToWordBoundary(
      `Покажу, что было до, какие действия сработали и какой результат получили в теме ${focus}.`,
      148
    );
  }
  ...
  if (mode === "social") {
    return trimToWordBoundary(
      `Коротко и по делу: почему вокруг темы ${focus} бывает хаос и что реально помогает.`,
      148
    );
  }

  return trimToWordBoundary(
    `Разберём причины в теме ${focus}: что происходит, что менять и какой шаг сделать первым.`,
    148
  );
}
...
function countPrimaryCtaActions(value: string) {
  const matches = normalizeText(value, 260).match(
    /(?:^|[^\p{L}])(напиши|напишите|сохраните|оставьте|отправьте|ответьте|пришлите|подпишитесь|выберите)(?=$|[^\p{L}])/giu
  );
...
function buildDomainSoftCta(domain: TopicDomain, focus: string, topicSeed?: string) {
  const variantsByDomain: Record<TopicDomain, string[]> = {
    ...
    creator: [
      ...
      `Сохраните разбор по ${focus} для следующей публикации.`
    ],
    sales: [
    ...
    general: [
      ...
      `Сохраните разбор по ${focus} для спокойной проверки.`
    ]
  };
