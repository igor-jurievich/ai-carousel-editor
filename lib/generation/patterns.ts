...
export const FIRST_SLIDE_SUBTITLE_BANNED_PATTERNS: RegExp[] = [
  /(?:^|[^\p{L}])покаж(?:ем|у)\s+причин[а-яё]*(?=$|[^\p{L}])/iu,
  /(?:^|[^\p{L}])механизм\s+и\s+перв(?:ый|ого)\s+шаг[а-яё]*(?=$|[^\p{L}])/iu,
  /(?:^|[^\p{L}])котор(?:ый|ого)\s+можно\s+проверить\s+без\s+давлени[а-яё]*(?=$|[^\p{L}])/iu,
  /(?:^|[^\p{L}])разбер[её]м\s+тему\s+подробно(?=$|[^\p{L}])/iu,
  /(?:^|[^\p{L}])в\s+теме(?=$|[^\p{L}])/iu,
